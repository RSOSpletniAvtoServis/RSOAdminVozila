...
from . import upoprigrpc_pb2
from . import upoprigrpc_pb2_grpc
...
class Kraji2(BaseModel):
    ids: List[int]
    uniqueid: str

@app.post("/izbranikraji/")
def get_izbranikraji(kraji2: Kraji2):
    print(kraji2.ids)     # list[int]
    print(kraji2.uniqueid)  # str
    ids_string = "("
    idmiddle = ",".join(str(i) for i in kraji2.ids)
...
def get_vozila(voz: Voz2):
    try:
        with pool.get_connection() as conn:
            with conn.cursor() as cursor:
                sql = "SELECT v.StevilkaSasije, v.LetoPrveRegistracije, v.KonjskaMoc, v.IDModel, m.NazivModel, v.IDZnamka, z.NazivZnamke, v.Aktiven FROM Vozilo v, Model m, Znamka z WHERE v.IDModel = m.IDModel AND z.IDZnamka = v.IDZnamka AND v.IDUporabnik = %s"
